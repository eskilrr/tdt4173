...
writeFile = "billboard-hits-2000-2020v3.csv"
#Get unique artists in list from billboard
df = pd.read_csv("CSV-files/OLD_FILES/billboard-hits-2000-2020v2.csv")
df['lead_artist_name'] = df['artist']
for i, row in df.iterrows():
    if row.artist == "Various Artists":
        a = sa.getArtistName(row.artist_id)
        df.at[i, 'lead_artist_name'] = a
        print(row['name'], row['artist'], a)
